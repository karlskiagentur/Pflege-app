...
export default function App() {
  ...
  const handleNewTerminRequest = async () => {
      ...
      try {
        ...
        setTimeout(() => { setActiveModal(null); setSentStatus('idle'); setRequestDate(""); setRequestReason(""); }, 1500);
      } catch (e) { setSentStatus('error'); }
      setIsSending(false);
  };

  // --- UPLOAD & URLAUB SUBMIT ---
  const submitData = async (type: string, payload: string) => {
    ...
    try {
      ...
      setTimeout(() => { 
          ...
          setSentStatus('idle'); 
          setUrlaubStart(""); setUrlaubEnde(""); setSelectedFiles([]); 
      }, 1500);
  ...
  return (
    <div className="min-h-screen bg-white pb-32 text-left select-none font-sans text-[#3A3A3A]" onMouseMove={handleDrag} onTouchMove={handleDrag} onMouseUp={() => isDragging.current = false} onTouchEnd={() => isDragging.current = false}>
      ...
      {/* MODALS */}
      {activeModal && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center p-4 animate-in fade-in"><div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => setActiveModal(null)}></div>
