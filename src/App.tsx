...
// DER SPÜRHUND: Findet Daten egal ob flach oder in fields
const getValue = (item: any, fieldName: string) => {
    if (!item) return "";
    // 1. Direkt (vom n8n Code Node)
    if (item[fieldName] !== undefined) return unbox(item[fieldName]);
    // 2. In fields (Airtable Standard)
  ...
  const fetchData = async (force = false, background = false) => {
    ...
    try {
        ...
        if (json.data) {
            ...
            console.log("Anzahl Besuche empfangen:", bData.length);
            ...
            console.log("Anzahl Tasks empfangen:", tData.length);
            ...
            console.warn("JSON ungültig:", json);
        }
