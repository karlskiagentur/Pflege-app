...
export default function App() {
  ...
  const handleDrag = (e: any) => {
    ...
    // Berechne Position vom rechten unteren Rand
    setKiPos({
  ...
  return (
    ...
    >
      <style>{`
        ...
        input[type="date"] { position: relative; min-height: 3.5rem; color: #3A3A3A !important; -webkit-appearance: listbox !important; }
      `}</style>
      ...
      <main className="max-w-md mx-auto px-6 pt-8">
        {/* TABS (Inhalt identisch mit vorherigem Stand) */}
        {activeTab === 'dashboard' && (
        ...
        {activeTab === 'service' && (
          <div className="space-y-6 animate-in fade-in text-left">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100 mt-4 text-left">
              ...
              <button onClick={() => submitService('Sonstiges')} disabled={isSending || !sonstigesMessage.trim()} className="w-full bg-[#dccfbc] text-white py-5 rounded-2xl font-black uppercase shadow-md disabled:bg-gray-100">Nachricht senden</button>
            </div>
          </div>
        )}
      </main>
